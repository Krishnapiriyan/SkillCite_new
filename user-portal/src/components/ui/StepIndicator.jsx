  ...
  return (
    <div className="w-full flex flex-col gap-3 py-4 select-none">
      ...
      <div className="relative w-full h-1.5 bg-border rounded-full overflow-hidden">
        <motion.div
          className="absolute inset-y-0 left-0 bg-purple-900 rounded-full"
          initial={{ width: '0%' }}
        ...
        {Array.from({ length: totalSteps }).map((_, idx) => {
          ...
          return (
            <div key={idx} className="flex flex-col items-center gap-1.5 relative z-10">
              <motion.div
                animate={{
                  backgroundColor: isCompleted || isActive ? '#581c87' : '#FFFFFF',
                  borderColor: isCompleted || isActive ? '#581c87' : '#E5E7EB',
              ...
              {stepNames[idx] && (
                <span className={`text-[10px] font-semibold tracking-wider uppercase
                  ${isActive ? 'text-purple-600' : isCompleted ? 'text-primary/70' : 'text-hint'}`}
                >
                  {stepNames[idx]}
                </span>
              )}
